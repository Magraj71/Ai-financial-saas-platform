...
export const summaryAnalyticsService = async (
  ...
  // Ensure current has safe defaults
  const {
    totalIncome = 0,
    totalExpenses = 0,
    availableBalance = 0,
  ...
  if (from && to && rangeValue !== DateRangeEnum.ALL_TIME) {
    // last N days previous last N days
    ...
    // SAFE: ensure previous has numeric defaults so we never pass undefined
    const safePrev = previous || { totalIncome: 0, totalExpenses: 0 };

    const prevIncome = safePrev.totalIncome || 0;
    const prevExpenses = safePrev.totalExpenses || 0;
    ...
    percentageChange: {
      ...
      previousValues: {
        incomeAmount: convertToDollarUnit(
          percentageChange.previousValues.incomeAmount || 0
        ),
        expenseAmount: convertToDollarUnit(
          percentageChange.previousValues.expenseAmount || 0
        ),
        balanceAmount: convertToDollarUnit(
          percentageChange.previousValues.balanceAmount || 0
        ),
      },
    },
    preset: {
...
export const chartAnalyticsService = async (
  ...
  const transaformedData = (resultData?.chartData || []).map((item: any) => ({
    date: item.date,
    income: convertToDollarUnit(item.income || 0),
    expenses: convertToDollarUnit(item.expenses || 0),
  }));
...
export const expensePieChartBreakdownService = async (
  ...
  const transformedData = {
    totalSpent: convertToDollarUnit(data.totalSpent || 0),
    breakdown: (data.breakdown || []).map((item: any) => ({
      ...item,
      value: convertToDollarUnit(item.value || 0),
    })),
  };
